...
- [ ] **P0-2c · Reproduce Cole's baseline** — process his 19 Aug demo-model video, confirm the 33 mm reading reproduces. *First real video (Aaron, 27 mm card, 2026-08-26) measured 32.98 mm end to end — see decisions.md D17; caliper truth pending.*
...
- [ ] **P1-5 · ⚑ COLMAP quality gate (timeboxed 3 days)** — run all fixture videos through COLMAP → downstream stages; judge base-perimeter deviation vs caliper truth. Within ±1 mm → COLMAP confirmed. Miss → activate P1-9 (Mac worker) and log in decisions.md. Cheap side-by-side with Meshroom/AliceVision while the harness is up. *Protocol + gate criteria: docs/test-protocol.md (2026-08-26). Runs on the admin bench (`/admin`) with widest + narrowest caliper truths; starts when the GPU droplet is up.*
...
- [ ] **P1-10 · End-to-end pipeline test** — video POSTed to API returns scaled outline + G-code matching legacy output on the same input. This closes P1. *(in progress 2026-08-26 — measurement software + queue wiring landed and then **review-fixed** (decisions.md D16): `app/measure/measure_scan.py` (robust marker triangulation with distortion → scale → marker+skin "up" → ROI crop → auto-slice above the skin plane → exact diameter → true-buffer FR-07 outline → wafer G-code per dialect), `app/measure_stage.py` (engine-neutral: mesh + `poses.json` + keyframes), `worker-colmap/reconstruct.py` returning mesh + poses. A job now ends at `measured` (G-code stored as `<id>/wafer.gcode`); `done` is the cut. Validated on synthetic ground truth incl. junk geometry, lens distortion and a bad view (33 mm ± 0.3). Remaining to close: real-video parity vs legacy, blocked on fixtures (P0-3); live run on the deployed CPU worker with the rebuilt image.)*
...
- [ ] **P4-1 · GRBL simulator target** — stand up grblHAL sim (or GRBL on a virtual serial port) as the development target. Software only, no hardware. *Ready to start: the wafer program is already emitted in the `grbl` dialect (`measure/gcode.py::GRBL`, config `GCODE_DIALECT`) and stored at `<job>/wafer.gcode`; the cutting stage claims `measured → cutting → done` via the generic `claim_next_job` (D16).*
